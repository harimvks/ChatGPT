...
from .artifact import CandidateArtifact, normalize_artifact
...
class SandboxEvaluator:
    """Normalize, materialize, evaluate, and remove a candidate workspace."""

    def __init__(self, sandbox: CandidateSandbox, gate: ExternalValidationGate) -> None:
        self._sandbox = sandbox
        self._gate = gate

    def evaluate(self, task: EngineeringTask, artifact: Mapping[str, str] | CandidateArtifact) -> SandboxEvaluation:
        normalized = normalize_artifact(artifact)
        workspace = self._sandbox.materialize(normalized.files)
        try:
            results = self._gate.run(workspace.path)
            checks = self._gate.checks(results)
            passed = all(checks.values())
            reward = sum(1.0 for value in checks.values() if value) / len(checks) if checks else 0.0
            failure_class = None
            if not passed:

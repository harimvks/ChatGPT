...
class RuntimeVNext:
    ...
    def execute(self, run: AgentRun, skill: GreenSkill, *, template: str) -> RuntimeResult:
        ...
        events.append(EvidenceRecord(current.run_id, "context_ready", current.status.value, (("context_id", manifest.context.context_id), ("context_hash", manifest.context.content_hash))))
        ...
        response = self._adapter.generate(AdapterRequest(capability_name=skill.name, capability_version=skill.version, capability_tag=skill.capability, template=template, context_manifest=manifest, context_id=manifest.context.context_id, context_hash=manifest.context.content_hash, classification=manifest.classification))
        current = replace(current, status=RunStatus.VALIDATING)
        events.append(EvidenceRecord(current.run_id, "validating", current.status.value, (("model", response.model), ("provider", response.provider_name))))
        ...
        events.append(EvidenceRecord(current.run_id, "accepted", current.status.value, (("execution_id", response.execution_id or ""), ("failover", ",".join(response.failed_over_from)))))

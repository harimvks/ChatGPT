...
class AgentRun:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.task_ref.strip():
            raise ValueError("task_ref cannot be empty")
        if self.child_budget_tokens < 0 or self.child_budget_count < 0:
            raise ValueError("child budgets cannot be negative")
        if self.max_subagent_depth < 0:
            raise ValueError("max_subagent_depth cannot be negative")


def derive_child_policy(parent: AgentRun, *, allow_write: bool = False) -> TaskPolicy:
    """Return a child policy that is never broader than the parent's authority."""
    return TaskPolicy(
